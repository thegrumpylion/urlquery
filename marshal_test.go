...
func TestMarshalScalar(t *testing.T) {
	...
	{
		ret, err := MarshalName("test ?with &stuff", "string")
		require.Nil(err)
		require.Equal("string=test+%3Fwith+%26stuff", ret)
	}
	{
		ret, err := MarshalName(true, "bool")
		require.Nil(err)
		require.Equal("bool=true", ret)
	}
	{
		ret, err := MarshalName(42, "int")
		require.Nil(err)
		require.Equal("int=42", ret)

	}
	{
		ret, err := MarshalName(3.14, "float")
		require.Nil(err)
...
func TestMarshalStructIgnoreKeys(t *testing.T) {
	require := require.New(t)
	...
	ret, err := Marshal(s)
	require.Nil(err)
	require.Equal("host=localhost&valid=true", ret)
}
